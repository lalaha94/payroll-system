...
function SalesDataDashboard() {
  ...
  const [unapprovedSales, setUnapprovedSales] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [salesResponse, modelsData, approvalsResponse] = await Promise.all([
        ...
        if (approvalsResponse.error) throw new Error(approvalsResponse.error.message);
        ...
        const unapproved = approvalsResponse.data.filter(sale => !sale.is_approved);
        const approved = approvalsResponse.data.filter(sale => sale.is_approved);
    ...
    <Box sx={{ 
      ...
      <TableContainer component={Paper} sx={{ mb: 4 }}>
        <Table>
          ...
          <TableBody>
            {unapprovedSales.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} align="center">
                  Ingen salg mangler godkjenning.
                </TableCell>
              </TableRow>
            ) : (
              unapprovedSales.map((sale, index) => (
                <TableRow key={index}>
                  <TableCell>{sale.agent_name || 'Ukjent'}</TableCell>
                  <TableCell>{sale.agent_company || 'Ukjent'}</TableCell>
                  <TableCell>{sale.total_premium?.toLocaleString('nb-NO')} kr</TableCell>
                  <TableCell>{sale.commission_amount?.toLocaleString('nb-NO')} kr</TableCell>
                  <TableCell>
                    <Chip label="Mangler godkjenning" color="warning" size="small" />
                  </TableCell>
                </TableRow>
              ))
            )}
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Premium</TableCell>
              <TableCell>Provisjon</TableCell>
              <TableCell>Godkjent av</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {approvedSales.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} align="center">
                  Ingen salg er godkjent.
                </TableCell>
              </TableRow>
            ) : (
              approvedSales.map((sale, index) => (
                <TableRow key={index}>
                  <TableCell>{sale.agent_name || 'Ukjent'}</TableCell>
                  <TableCell>{sale.agent_company || 'Ukjent'}</TableCell>
                  <TableCell>{sale.total_premium?.toLocaleString('nb-NO')} kr</TableCell>
                  <TableCell>{sale.commission_amount?.toLocaleString('nb-NO')} kr</TableCell>
                  <TableCell>{sale.approved_by || 'Ukjent'}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
