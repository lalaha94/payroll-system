  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      if (authData?.session?.user) {
        ...
        console.log("AgentDashboard - Employee data:", employeeData);
        console.log("AgentDashboard - Employee error:", employeeError);
        ...
        if (employeeData) {
          ...
          const { data: emailEmployeeData, error: emailError } = await supabase
            .from('employees')
            .select('*')
            .eq('email', user.email)
            .single();
